...
def build_pitch_shapes() -> list[dict]:
    """
    Draw a vertical full-pitch layout to match the user's app screenshots.

    Coordinate system used by the app:
      x: 0 -> 100   left to right
      y: 0 -> 100   top to bottom

    Plotly is also drawn in that same screen-style orientation by reversing y-axis.
    The geometry below is approximated to visually match the screenshots rather than
    to be a metrically perfect GAA CAD drawing.
    """
    shapes: list[dict] = []
    ...
    # Main horizontal pitch lines to match screenshot spacing
    for y in [9, 28.5, 42.2, 48.0, 52.0, 71.5, 91.0]:
        shapes.append(dict(type="line", x0=0, y0=y, x1=100, y1=y, line=line))

    # Top large rectangle
    shapes.append(dict(type="rect", x0=39.5, y0=0, x1=60.5, y1=9, line=line))
    # Top small rectangle
    shapes.append(dict(type="rect", x0=42.2, y0=0, x1=57.8, y1=3.0, line=line))
    # Top goal
    shapes.append(dict(type="line", x0=46.3, y0=0.8, x1=46.3, y1=0, line=line))
    shapes.append(dict(type="line", x0=53.7, y0=0.8, x1=53.7, y1=0, line=line))

    # Bottom large rectangle
    shapes.append(dict(type="rect", x0=39.5, y0=91, x1=60.5, y1=100, line=line))
    # Bottom small rectangle
    shapes.append(dict(type="rect", x0=42.2, y0=97.0, x1=57.8, y1=100, line=line))
    # Bottom goal
